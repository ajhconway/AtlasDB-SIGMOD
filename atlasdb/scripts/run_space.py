...
config_options = "--db-location /dev/nvme0n1 --set-O_DIRECT --unset-O_CREAT --memtable-capacity-mib 24 --fanout 8 --max-branches-per-node 24 --stats"
...
def get_trace_dir_name(key_size, value_size):
    ...
    if value_size == 100:
        return "/mnt/data/traces"
    else:
        return "/mnt/traces/80gib_24b_1kib"
...
def get_workload_string(phase, threads, mem, value_size, reclaim_threshold):
    if reclaim_threshold == -1:
        rt_str = "inf"
    else:
        rt_str = str(reclaim_threshold)
    return phase + "-value_" + str(value_size) + "-thr_" + str(threads) + "-mem_" + bytes_to_str(mem) + "-rt_" + rt_str
...
class Benchmark:
    ...
    def __init__(self, name, phases, threads, mem, capacity, key_size, value_size, routing, log=1, rec_thresh=-1):
        self.name = name
        assert(phases)
        self.phases = phases
        assert(threads)
            ...
            for threads in self.threads:
                ...
                with open(ycsb_csv_name, "w") as ycsb_csv:
                    ...
                    for phase in self.phases:
                        ...
                        workload_string = get_workload_string(phase, threads, mem, self.value_size, self.rec_thresh)
                        ...
                        trace_filename = self.trace_dir_name + "/ycsb_" + phase
                        ...
                        run_command = ["cgexec", "-g", "memory:benchmark", "bin/driver_test", "ycsb_test"]
                        ...
                        if (self.rec_thresh != -1):
                            run_command.append("--reclaim-threshold-gib")
                            run_command.append(str(self.rec_thresh))
...
#lookup_load = Benchmark("lookup", ["load"], [20], [splintersize], 256, 24, 1024, 1)
#lookup = Benchmark("lookup", ["c", "u"], [8, 12, 16, 20, 24, 28, 32], [small, medium, kvellsize, large], 256, 24, 1024, 1)
#lookup_load.run("/home/XXXX/sosp-data/atlas")
#lookup.run("/home/XXXX/sosp-data/atlas")

#dbsize = 320
#
#ycsb = Benchmark("ycsb", ["load", "a", "b", "c", "u", "d", "f", "e"], [8, 12, 16, 20], [splintersize], dbsize, 24, 1024, 1)
#ycsb.run("/home/XXXX/sosp_data/atlas")

#dbsize = 256
#
#ycsb = Benchmark("ycsb", ["load", "a", "b", "c", "d", "f", "e"], [8], [splintersize, kvellsize], dbsize, 24, 100, 1)
#ycsb.run("/home/XXXX/sosp_data/atlas")

#space = Benchmark("space", ["load", "uniform_update"], [16], [kvellsize], 256, 24, 100, 1, log = 0, rec_thresh=0)
#space.run("/home/XXXX/quick")
space = Benchmark("space", ["load", "uniform_update"], [8], [kvellsize], 256, 24, 100, 1, log = 0, rec_thresh=96)
space.run("/home/XXXX/quick")
space = Benchmark("space", ["load", "uniform_update"], [8], [kvellsize], 256, 24, 100, 1, log = 0, rec_thresh=104)
space.run("/home/XXXX/quick")
space = Benchmark("space", ["load", "uniform_update"], [8], [kvellsize], 256, 24, 100, 1, log = 0, rec_thresh=112)
space.run("/home/XXXX/quick")
space = Benchmark("space", ["load", "uniform_update"], [8], [kvellsize], 256, 24, 100, 1, log = 0, rec_thresh=120)
space.run("/home/XXXX/quick")
space = Benchmark("space", ["load", "uniform_update"], [8], [kvellsize], 256, 24, 100, 1, log = 0, rec_thresh=-1)
space.run("/home/XXXX/quick")

#space = Benchmark("space", ["load", "zipf_update"], [16], [medium], 256, 24, 1024, 1, log = 0, rec_thresh=0)
#space.run("/home/XXXX/sosp_data/atlas")
#space = Benchmark("space", ["load", "zipf_update"], [16], [medium], 256, 24, 1024, 1, log = 0, rec_thresh=125)
#space.run("/home/XXXX/sosp_data/atlas")
#space = Benchmark("space", ["load", "zipf_update"], [16], [medium], 256, 24, 1024, 1, log = 0, rec_thresh=135)
#space.run("/home/XXXX/sosp_data/atlas")
#space = Benchmark("space", ["load", "zipf_update"], [16], [medium], 256, 24, 1024, 1, log = 0, rec_thresh=145)
#space.run("/home/XXXX/sosp_data/atlas")
#space = Benchmark("space", ["load", "zipf_update"], [16], [medium], 256, 24, 1024, 1, log = 0, rec_thresh=155)
#space.run("/home/XXXX/sosp_data/atlas")
#space = Benchmark("space", ["load", "zipf_update"], [16], [medium], 256, 24, 1024, 1, log = 0, rec_thresh=-1)
#space.run("/home/XXXX/sosp_data/atlas")
