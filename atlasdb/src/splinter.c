                         ...
                         bool            *found)
{
   ...
   if (local_found) {
      *found = TRUE;
      ...
      btree_node_unget(cc, cfg, &node);
      message_type type = data_class(data);
                                ...
                                page_handle                 *node)
{
   ...
   for (uint16 pivot_no = 0; pivot_no < num_children; pivot_no++) {
      ...
      if (pdata->srq_idx != -1 && spl->cfg.reclaim_threshold != UINT64_MAX) {
         //platform_log("Updating %12lu-%lu:%8lu into SRQ (index %lu)\n",
         //   node->disk_addr, pdata->generation, num_tuples_to_reclaim, pdata->srq_idx);
         ...
         //platform_log("Inserting %12lu-%lu:%8lu into SRQ (index %lu)\n",
         //      data.addr, data.pivot_generation, data.priority, pdata->srq_idx);
      }
   }
}
                              ...
                              void *scratch)
{
   ...
   do {
      ...
      if (splinter_build_filter_should_reenqueue(req, node)) {
         task_enqueue(spl->ts, TASK_TYPE_NORMAL, splinter_bundle_build_filters,
               req, FALSE);
                     ...
                     uint64           pivot_no)
{
   ...
   for (uint16 pivot_no = 0; pivot_no < right_num_children; pivot_no++) {
      ...
      if (pdata->srq_idx != -1 && spl->cfg.reclaim_threshold != UINT64_MAX) {
         ...
         //platform_log("Reinserting %12lu-%lu into SRQ (index %lu)\n",
         //      right_addr, pdata->generation, pdata->srq_idx);
      }
   }
                       ...
                       bool            *found)
{
   ...
   if (spl->cfg.use_stats) {
      ...
      spl->stats[tid].filter_lookup_time_ns[height] +=
   ...
   while (next_value != ROUTING_NOT_FOUND) {
      uint64 branch_lookup_start;
      if (spl->cfg.use_stats) {
         branch_lookup_start = platform_get_timestamp();
      }
      uint16 branch_no =
      ...
      if (spl->cfg.use_stats) {
         ...
         spl->stats[tid].branch_lookup_time_ns[height] +=
            platform_timestamp_elapsed(branch_lookup_start);
      }
      if (local_found) {
         if (data_class(data) != MESSAGE_TYPE_UPDATE) {
            return FALSE;
         }
        ...
        branch_no = splinter_branch_no_add(spl, branch_no, 1))
   {
           ...
           sb_no = splinter_subbundle_no_add(spl, sb_no, 1))
      {
         ...
         if (branch_no == sb->start_branch) {
            ...
            if (filter_count > 0) {
               routing_filter *filter =
                  splinter_subbundle_filter(spl, node, sb, 0);
               sprintf(filter1_str, "%12lu", filter->addr);
            } else {
               sprintf(filter1_str, "%12s", "");
            }
            char filter2_str[13];
            if (filter_count > 1) {
               routing_filter *filter =
                  splinter_subbundle_filter(spl, node, sb, 1);
               sprintf(filter2_str, "%12lu", filter->addr);
            } else {
               sprintf(filter2_str, "%12s", "");
            }
            char filter3_str[13];
            if (filter_count > 2) {
               routing_filter *filter =
                  splinter_subbundle_filter(spl, node, sb, 2);
               sprintf(filter3_str, "%12lu", filter->addr);
            } else {
               sprintf(filter3_str, "%12s", "");
            }
            char *extra_filter_str;
            if (filter_count > 3) {
               extra_filter_str = " *";
            } else {
               extra_filter_str = "  ";
            ...
            if (sb->state == SB_STATE_COMPACTED) {
               platform_log_stream("|     |   -- comp subbundle %2u --   | %s | %s | %s | %s |\n",
                     sb_no, filter1_str, filter2_str, filter3_str, extra_filter_str);
            } else {
               platform_log_stream("|     |  -- uncomp subbundle %2u --  | %s | %s | %s | %s |\n",
                     sb_no, filter1_str, filter2_str, filter3_str, extra_filter_str);
            }
         }
      }
                      ...
                      char            *key)
{
   ...
   for (uint16 h = height; h > 0; h--) {
      ...
      if (found) {
         char key_str[128];
         splinter_key_to_string(spl, key, key_str);
         platform_log("Key %s found in node %lu pivot %u with data %d:%d\n",
               key_str, node->disk_addr, pivot_no,
               ((data_handle *)data)->message_type,
               ((data_handle *)data)->ref_count);
         found = FALSE;
      } else {
         for (uint16 branch_no = pdata->start_branch;
              branch_no != splinter_end_branch(spl, node);
   ...
   if (found) {
      ...
      for (uint16 branch_no = pdata->start_branch;
         ...
         if (found) {
            ...
            platform_log("!! Key %s found in branch %u of node %lu pivot %u "
                  "with data %d:%d\n",
                  key_str, branch_no, node->disk_addr, 0,
                  ((data_handle *)data)->message_type,
                  ((data_handle *)data)->ref_count);
         }
      }
                     ...
                     uint64           use_stats)
{
   ...
   index_filter_cfg->hash       = platform_checksum32;
