...
#else
  /*
   * You're allowed to turn on SPLINTER_DEBUG without VMX86_DEBUG
   * (to have extra asserts in release mode)
   */
#  if SPLINTER_DEBUG != 0 && SPLINTER_DEBUG != 1
#     error SPLINTER_DEBUG not 0 or 1
#  endif
#  if defined(VMX86_DEBUG) && SPLINTER_DEBUG == 0
#     error VMX86_DEBUG with SPLINTER_DEBUG==0 not supported
#  endif
#endif
