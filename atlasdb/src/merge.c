                      ...
                      data_config *cfg)
{
   ...
   for (int i = insert_position; i < stack->size; ++i) {
      ...
      if (cfg->key_compare(stack->limits[stack->seq[i]],
                           limit, cfg->key_size) > 0) {
                                   ...
                                   debug_only data_config *cfg)
{
    ...
    if (stack->size > 0) {
       ...
       int seq = stack->seq[0];
       debug_assert(cfg->key_compare(stack->start_key,
                                     stack->limits[seq], cfg->key_size) < 0);
    } else {
    ...
    for (int i = 1; i < stack->size; i++) {
       ...
       debug_assert(cfg->key_compare(stack->limits[seq_lo],
                                     stack->limits[seq_hi], cfg->key_size) < 0);
    }
#endif
}
                              ...
                              data_config *cfg)
{
   ...
   if (predecessor >= 0 &&
       cfg->key_compare(stack->limits[stack->seq[predecessor]],
                        limit, cfg->key_size) >= 0) {
                         ...
                         data_config *range_cfg)
{
   ...
   debug_assert(point_cfg->key_size <= point_cfg->data_size);
   debug_assert(range_cfg->key_size == range_cfg->data_size);
   ...
   switch (ordered_itor->type) {
      case data_type_point:
         if (range_cfg->clobber_message_with_range_delete) {
            range_cfg->clobber_message_with_range_delete(ordered_itor->key,
                                                   ordered_itor->data);
         }
         break;
...
merge_resolve_equal_keys(merge_iterator *merge_itor, bool *retry) {
                                      ...
                                      merge_itor->ordered_iterators[0]->seq))
   {
      ...
      memmove(merge_itor->merge_buffer, merge_itor->data, cfg->data_size);
      ...
      do {
                                 ...
                                 merge_itor->ordered_iterators[1]->seq))
         {
            ...
            cfg->merge_tuples(merge_itor->key,
      ...
      message_type class = cfg->data_class(merge_itor->data);
      /*
...
merge_resolve_updates_and_discard_deletes(merge_iterator *merge_itor)
{
   ...
   if (class != MESSAGE_TYPE_INSERT && merge_itor->resolve_updates) {
      ...
      cfg->merge_tuples_final(merge_itor->key,
                              (data_handle *)merge_itor->data);
      class = cfg->data_class((data_handle *)merge_itor->data);
   }
