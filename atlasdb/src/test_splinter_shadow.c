 ...
 * db. This is used during functionality tests to validate against splinter.
 * This Uses a generic AVL Tree as the backing data structure. The usage of
 * this is expectd to be in cases where performance is not of paramount
 * importance. Also this implementatio is not thread safe.
 *
 *
 * EXPLANATION for why we expect the recursive algorithm used in the backing
 * avlTree data structure to work.
 *
 * -- We ran a test where the test only did inserts. The number of operations
 * in the test were 140928614 (140 million) which is around 1.4 times the
 * maximum operations that the test will do. (96 million). During this test
 * the max height of the avlTree grew to 30, thereby causing 30 stack frames
 * in the recursion. The theoritical max for an avlTree is 1.44 * log2(n).
 * This would mean at max 39 stack frames.
 *
 * The test completed successfully with 20 KiB of stack, This would mean at
 * max with 39 stack frames, we would need (20/30) * 39 = 26 KiB oi stack.
 *
 * The general permissible stack growth limit in the kernel is 8 pages, so
 * 32 KiB. This leaves us with extra leeway.
 */
#include "platform.h"
