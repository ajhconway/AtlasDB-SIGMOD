                ...
                uint64      generation)
{
   ...
   data_cursor = cursor;
   cursor += log->cfg->data_cfg->data_size;
   thread_data->offset += log->cfg->data_cfg->data_size;
   ...
   *generation_cursor = generation;
   ...
   memmove(data_cursor, data, log->cfg->data_cfg->data_size);
                           ...
                           uint64            extent_size)
{
   ...
   log_entry_size
      = data_cfg->key_size + data_cfg->data_size + sizeof(uint64);
      ...
      for (i = 0; i < pages_per_extent; i++) {
         ...
         if (shard_log_valid(cfg, page, magic)) {
            ...
            for (j = 0; j < log->cfg->entries_per_page; j++) {
               ...
               data = cursor;
               cursor += log->cfg->data_cfg->data_size;
               data_key_to_string(key, key_str, 128);
               data_data_to_string(data, data_str, 128);
