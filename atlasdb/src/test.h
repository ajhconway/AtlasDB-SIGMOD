   ...
   for (i = 0; i < num_trees; i++) {
      ...
      while (!at_end) {
         ...
         if (last_key != NULL && btree_key_compare(cfg, last_key, key) > 0) {
            char last_key_str[MAX_KEY_SIZE], key_str[MAX_KEY_SIZE];
         ...
         if (btree_key_compare(cfg, low_key, key) > 0) {
            char low_key_str[MAX_KEY_SIZE], key_str[MAX_KEY_SIZE], high_key_str[MAX_KEY_SIZE];
                 ...
                 master_config       *master_cfg)
{
   ...
   data_cfg->data_size          = master_cfg->data_size;
   data_cfg->key_compare        = memcmp;
   data_cfg->key_to_string      = data_key_to_string;
   data_cfg->data_to_string     = data_data_to_string;
   data_cfg->merge_tuples       = data_merge_tuples;
   data_cfg->merge_tuples_final = data_merge_tuples_final;
   data_cfg->clobber_message_with_range_delete = NULL;
   data_cfg->data_class         = data_class;
   memset(data_cfg->min_key, 0, data_cfg->key_size);
   memset(data_cfg->max_key, 0xff, data_cfg->key_size);
...
static splinter_id counter = 1;

static inline splinter_id
test_generate_splinter_id()
{
