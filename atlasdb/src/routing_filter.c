 ...
 * RadixSort --
 *
 *      A fast integer sort obtained from Stack Overflow (TODO: find reference for this)
 *
 *----------------------------------------------------------------------
 */
...
{
   uint64 addrs_per_page = cfg->page_size / sizeof(uint64);
   ...
   uint64 header_addr = hdr_raw_addr - (hdr_raw_addr % cfg->page_size);
...
routing_header_length(routing_config *cfg,
                      routing_hdr    *hdr)
{
   uint64 metadata_size = (hdr->num_remainders + cfg->index_size - 1) / 8 + 4;
   return metadata_size + sizeof(routing_hdr);
}
...
void
routing_get_bucket_counts(routing_config *cfg,
                          routing_hdr    *hdr,
                          uint32         *count)
{
   uint64 start = 0;
                   ...
                   uint16            value)
{
   ...
   // for convenience
   uint64  page_size        = cfg->page_size;
   uint64  extent_size      = cfg->extent_size;
   ...
   uint32 *fp_arr = RadixSort(new_fp_arr, matrix, temp, num_new_fp,
         cfg->fingerprint_size, value_size);
   ...
   for (uint32 src_fp_no = 0; src_fp_no != num_new_fp; src_fp_no++) {
      ...
      if (dst_fp_no == 0 ||
            fp_arr[dst_fp_no] != fp_arr[dst_fp_no - 1]) {
         dst_fp_no++;
      } else {
   ...
   for (uint32 index_no = 0; index_no < num_indices; index_no++) {
      ...
      for (; fp_no < num_new_unique_fp &&
            routing_get_index(fp_arr[fp_no], index_remainder_and_value_size) == index_no;
            fp_no++) { }
   ...
   for (uint32 old_index_no = 0; old_index_no < old_num_indices; old_index_no++) {
      ...
      if (old_filter->addr != 0) {
         routing_hdr *old_hdr =
            routing_get_header(cc, cfg, old_filter->addr, old_index_no,
                               &old_filter_node);
      ...
      if (old_index_count != 0) {
         PackedArray_unpack((uint32 *)old_block_start, 0,
               old_src_fp, old_index_count, old_remainder_and_value_size);
         uint32 old_fp_no = 0;
         for (uint32 bucket_off = 0; bucket_off < index_size; bucket_off++) {
            ...
            for (uint32 i = 0; i < old_count[bucket_off]; i++) {
               old_src_fp[old_fp_no++] |= bucket << old_remainder_and_value_size;
            }
         }
      ...
      for (uint32 index_off = 0; index_off < new_indices_per_old_index; index_off++) {
         ...
         while (new_fps_added < new_index_count || old_fps_added < old_index_count) {
            uint32 fp;
            bool is_old = (0 || new_fps_added == new_index_count
         ...
         if (fps_added != 0) {
            PackedArray_pack((uint32 *)filter_cursor, 0, fp_buffer, fps_added,
                        ...
                        uint64          num_indices)
{
   ...
   for (uint64 index_page_no = 0;
        index_page_no < num_index_pages;
        index_page_no++)
   {
                                  ...
                                  uint64            num_filters)
{
   ...
   uint32 *local =
      TYPED_ARRAY_ZALLOC(hid, local, alloc_size * sizeof(uint32));
   uint32 *fp_arr = local;
   ...
   for (uint64 i = 0; i != num_filters; i++) {
      ...
      for (uint32 index_no = 0; index_no < num_indices; index_no++) {
         ...
         if (index_count != 0) {
            ...
            PackedArray_unpack((uint32 *)block_start, 0, src_fp, index_count,
                  remainder_and_value_size);
                      ...
                      uint64         *found_values)
{
   ...
   uint64 start, end;
   routing_get_bucket_bounds(hdr->encoding, header_length, bucket_off, &start,
         &end);
   ...
   for (uint32 i = 0; i < end - start; i++) {
      ...
      routing_filter_get_remainder_and_value(cfg,
            (uint32 *)remainder_block_start, pos, &found_remainder_and_value,
            remainder_and_value_size);
   ...
   do {
      ...
      case routing_async_state_start:
      {
         ...
         ctxt->page_addr = filter->addr +
            cfg->page_size * (ctxt->index / addrs_per_page);
      ...
      case routing_async_state_get_filter:
      {
         ...
         cache_ctxt_init(cc, routing_filter_async_callback, ctxt, cache_ctxt);
...
//                         __FILE__, __LINE__, platform_get_tid(), ctxt);
      ...
      case routing_async_state_got_filter:
      {
         ...
         routing_get_bucket_bounds(hdr->encoding, header_length, bucket_off,
               &start, &end);
         char *remainder_block_start = (char *)hdr + header_length;
         ...
         for (uint32 i = 0; i < end - start; i++) {
            ...
            size_t remainder_and_value_size = ctxt->remainder_size + value_size;
            routing_filter_get_remainder_and_value(cfg,
                  (uint32 *)remainder_block_start, pos,
                  &found_remainder_and_value, remainder_and_value_size);
            uint32 found_remainder =
               found_remainder_and_value >> value_size;
                                    ...
                                    routing_config *cfg)
{
   ...
   return routing_filter_estimate_unique_keys_from_count(cfg, filter->num_unique);
}
...
void
routing_filter_print_encoding(routing_config *cfg,
                              routing_hdr    *hdr)
{
   uint32 i;
                                ...
                                size_t          value_size)
{
   ...
   for (i = 0; i < cfg->index_size; i++) {
      ...
      for (j = start; j < end; j++) {
         ...
         routing_filter_get_remainder_and_value(cfg,
               (uint32 *)((char *)hdr + header_length), j, &remainder_and_value,
               remainder_and_value_size);
