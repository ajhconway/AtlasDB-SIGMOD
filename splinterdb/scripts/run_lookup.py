...
config_options = "--db-location /dev/nvme0n1p2 --set-O_DIRECT --unset-O_CREAT --memtable-capacity-mib 24 --fanout 8 --max-branches-per-node 24 --stats"
...
def get_trace_dir_name(key_size, value_size):
    ...
    if value_size == 100:
        return "/mnt/traces/80gib_24b_100b"
    else:
        return "/mnt/traces/80gib_24b_1kib"

# generate csv filename
def get_csv_string(save_dir, name, phase, datetime_string, mem, value_size):
    return "{}/{}-{}-value_{}-mem_{}-{}.csv".format(save_dir, name,
            phase, value_size, bytes_to_str(mem), datetime_string)
...
class Benchmark:
    ...
    def run(self, save_dir):
        ...
        #results_dir = "{}/{}_temp_load-value_{}-{}".format(save_dir, self.name, self.value_size, datetime_string)
        #pathlib.Path(results_dir).mkdir(parents=True, exist_ok=True)
        #write_version_info(results_dir)

        ## get trace filename and operation count
        #trace_filename = self.trace_dir_name + "/replay_load"
        ...
        #try:
        #    print("Running Load for {} with {} threads and {} RAM".format(self.name, 16, bytes_to_str(20 * 1024 * 1024 * 1024)))
        ...
        for mem in self.mem:
            for phase in self.phases:
                ...
                lookup_csv_name = get_csv_string(save_dir, self.name, phase,
                        datetime_string, mem, self.value_size);
                #lookup_csv_file = pathlib.Path(lookup_csv_name)
                #assert(not lookup_csv_file.is_file())
                with open(lookup_csv_name, "w") as lookup_csv:
                    lookup_csv.write("Threads Throughput MeanLatency 99PLatency MaxLatency ReadAmp WriteAmp\n")
                    for threads in self.threads:
                        ...
                        lookup_csv.write("{} {} {} {} {} {} {}\n".format(
                            threads, throughput, mean_latency, perc_latency,
                            max_latency, read_amp, write_amp))
...
#ycsb = Benchmark("lookup", ["c_long", "u_long"], [8, 12, 16, 20, 24, 28, 32], [medium, kvellsize, large], dbsize, 24, 100, 0)
#ycsb.run("/home/XXXX/sosp_data/splinter")

#biglookup = Benchmark("lookup", ["u_extralong"], [8, 12, 16, 20, 24, 28, 32], [120 * gib], dbsize, 24, 1024, 0)
#biglookup.run("/home/XXXX/sosp_data/splinter")

biglookup = Benchmark("lookup", ["c_long"], [8, 32], [120 * gib], dbsize, 24, 100, 0)
biglookup.run("/home/XXXX/sosp_data/splinter")
