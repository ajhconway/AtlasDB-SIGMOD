 ...
 * btree_test.c --
 *
 *     This file contains the test interfaces for the B-tree.
 */
#include "platform.h"
...
test_btree_data_equal(btree_config *cfg,
                      ...
                      const char   *data2)
{
   return memcmp(data1, data2, btree_data_size(cfg)) == 0;
}
                 ...
                 uint64                     thread_id)
{
   ...
   test_key(key, cfg->type, seq, thread_id, cfg->semiseq_freq,
         btree_key_size(cfg->mt_cfg->btree_cfg));
   if (data != NULL) {
      test_insert_data((data_handle *)data, 1, (char *)&seq, 8,
            btree_data_size(cfg->mt_cfg->btree_cfg), MESSAGE_TYPE_INSERT);
   }
}
...
test_btree_insert_thread(void *arg)
{
   ...
   char key[MAX_KEY_SIZE], data[MAX_DATA_SIZE];
   ...
   for (i = 0; i < max_async_inflight; i++) {
      ...
      case async_success:
         if (found ^ expected_found) {
            ...
            char key_string[128];
            cfg->data_cfg->key_to_string(ctxt->key, key_string,
                                         btree_key_size(cfg));
                        ...
                        bool                    *correct)
{
   ...
   memset(async_ctxt->data, 0, MAX_DATA_SIZE);
                 ...
                 uint64             num_inserts)
{
   ...
   uint64 start_time = platform_get_timestamp();
   char key[MAX_KEY_SIZE];
   char *data = TYPED_ARRAY_MALLOC(hid, data, MAX_DATA_SIZE);
   ...
   platform_log("btree insert time per tuple %lu\n",
   ...
   uint64 num_async = 0;
   char *expected_data = TYPED_ARRAY_MALLOC(hid, expected_data, MAX_DATA_SIZE);
   platform_assert(data != NULL);
   ...
   platform_log("btree positive lookup time per tuple %lu\n",
         platform_timestamp_elapsed(start_time) / num_inserts);
   ...
   platform_log("btree negative lookup time per tuple %lu\n",
         platform_timestamp_elapsed(start_time) / num_inserts);
   ...
   btree_iterator_init(cc, btree_cfg, &itor, root_addr, PAGE_TYPE_MEMTABLE,
                       ...
                       data_type_point);
   platform_log("btree iterator init time %lu\n", platform_timestamp_elapsed(start_time));
   btree_pack_req req;
   ...
   platform_log("btree itor/pack time per tuple %lu\n",
         platform_timestamp_elapsed(start_time) / num_inserts);
   cache_assert_free(cc);
   ...
   platform_log("btree packed positive lookup time per tuple %lu\n",
         platform_timestamp_elapsed(start_time) / num_inserts);
   platform_log("%lu%% lookups were async\n", num_async * 100 / num_inserts);
   cache_assert_free(cc);
   ...
   platform_log("btree packed negative lookup time per tuple %lu\n",
         platform_timestamp_elapsed(start_time) / num_inserts);
   cache_assert_free(cc);
                          ...
                          uint64             iterations)
{
   ...
   for (uint64 i = 0; i < iterations; i++) {
      test_key(min_key, TEST_RANDOM, 2*i, 0, 0, btree_key_size(btree_cfg));
      test_key(max_key, TEST_RANDOM, 2*i + 1, 0, 0, btree_key_size(btree_cfg));
