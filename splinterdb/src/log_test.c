 ...
 * log_test.c --
 *
 *     This file contains tests for the log
 */
#include "platform.h"
               ...
               platform_heap_id  hid)
{
   ...
   for (i = 0; i < num_entries; i++) {
      test_key(key, TEST_RANDOM, i, 0, 0, cfg->data_cfg->key_size);
      test_insert_data(data, 1, &dummy, 0, cfg->data_cfg->data_size, MESSAGE_TYPE_INSERT);
   ...
   for (i = 0; i < num_entries && !at_end; i++) {
      ...
      if (data_key_compare(cfg->data_cfg, key, returned_key) != 0
            || memcmp(data, returned_data, cfg->data_cfg->data_size) != 0) {
         platform_log("log_test_basic: key or data mismatch\n");
         cfg->data_cfg->key_to_string(key, key_str, 0);
         cfg->data_cfg->data_to_string(data, data_str, 0);
...
test_log_thread(void *arg)
{
   ...
   uint64 i;
   char key[MAX_KEY_SIZE];
   char *data = TYPED_ARRAY_MALLOC(hid, data, log->cfg->data_cfg->data_size);
   char dummy;

   for (i = thread_id * num_entries; i < (thread_id + 1) * num_entries; i++) {
      test_key(key, TEST_RANDOM, i, 0, 0, log->cfg->data_cfg->key_size);
      test_insert_data(data, 1, &dummy, 0, log->cfg->data_cfg->data_size, MESSAGE_TYPE_INSERT);
      log_write(logh, key, data, i);
   }
