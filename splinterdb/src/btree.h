...
typedef struct __attribute__ ((packed)) btree_hdr {
   uint64 next_addr;
...
static inline uint64
btree_data_size(btree_config *cfg)
{
   return cfg->data_cfg->data_size;
}

static inline uint64
btree_tuple_size(btree_config *cfg)
{
   return btree_key_size(cfg) + btree_data_size(cfg);
}

static inline uint64
btree_pivot_size(btree_config *cfg)
{
   return btree_key_size(cfg) + sizeof(uint64);
}
...
btree_key_compare(btree_config *cfg,
                  ...
                  const char   *key2)
{
   return cfg->data_cfg->key_compare(key1, key2, btree_key_size(cfg));
}
...
btree_key_to_string(btree_config *cfg,
                    ...
                    char *str)
{
   return cfg->data_cfg->key_to_string(key, str, btree_key_size(cfg));
}

static inline void
btree_data_to_string(btree_config *cfg,
                     const char *data,
                     char *str)
{
   return cfg->data_cfg->data_to_string(data, str, btree_data_size(cfg));
}
