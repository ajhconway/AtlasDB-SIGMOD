             ...
             bool             found)
{
   ...
   if (found && expected_data) {
      ...
      if (memcmp(expected_data, data, data_size) != 0) {
         splinter_data_to_string(spl, data, data_str);
      ...
      for (uint64 op_offset = 0; op_offset != op_granularity; op_offset++) {
         for (uint8 spl_idx = 0; spl_idx < num_tables; spl_idx++) {
            if (test_is_done(done, spl_idx))
               continue;
            splinter_handle *spl = spl_tables[spl_idx];
            tuple_size = splinter_key_size(spl) + splinter_data_size(spl);
            char *range_output =
      ...
      if (type == OP_INSERT) {
         ...
         if (random_next_uint64(rs) % 100 < lookup_positive_pct) {
            ...
            if (local_curr_op >= total_ops[spl_idx] - op_granularity) {
               ...
               random_base = (random_next_uint64(rs) / op_granularity) *
                             op_granularity + local_curr_op;
            } else {
                   ...
                   uint64            insert_rate)
{
   ...
   for (uint8 i = 0; i < num_tables; i++) {
      tuple_size = cfg[i].data_cfg->key_size + cfg[i].data_cfg->data_size;
   ...
   for (uint64 i = 0; i < num_lookup_threads; i++) {
      ...
      for (uint8 j = 0; j < num_tables; j++) {
         async_ctxt_init(hid, max_async_inflight, cfg[j].data_cfg->data_size,
...
test_splinter_shadow_insert(splinter_handle *spl,
                            ...
                            uint64           idx)
{
   uint64 byte_pos = idx * (splinter_key_size(spl) + splinter_data_size(spl));
   memmove(&shadow[byte_pos], key, splinter_key_size(spl));
   byte_pos += splinter_key_size(spl);
   memmove(&shadow[byte_pos], data, splinter_data_size(spl));
}
                    ...
                    platform_heap_id  hid)
{
   ...
   splinter_handle *spl =
      splinter_create(cfg, al, cc, ts, test_generate_splinter_id(), hid);
   platform_status rc;
   ...
   const size_t data_size = splinter_data_size(spl);
   ...
   for (insert_num = 0; insert_num < num_inserts; insert_num++) {
      ...
      test_key(key, TEST_RANDOM, insert_num, 0, 0, key_size);
      test_insert_data((data_handle *)data, 1, (char *)&insert_num,
   ...
   for (insert_num = 0; insert_num < num_inserts; insert_num++) {
      ...
      if (max_async_inflight == 0) {
         ...
         test_key(ctxt->key, TEST_RANDOM, insert_num, 0, 0, key_size);
   ...
   for (uint64 range_num = 0; range_num != num_ranges; range_num++) {
      ...
      if (returned_tuples != expected_returned_tuples
         ...
         for (uint64 i = 0; i < expected_returned_tuples; i++) {
            ...
            offset += key_size;
            splinter_data_to_string(spl, shadow_start + offset, expected_data);
            splinter_data_to_string(spl, range_output + offset, actual_data);
...
splinter_test(int argc, char *argv[])
{
   ...
   switch (test) {
      ...
      case functionality:
         for (uint8 i = 0; i < num_tables; i++) {
            splinter_cfg[i].data_cfg->key_to_string = functionality_key_to_string;
         }
