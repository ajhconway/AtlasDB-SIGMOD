...
btree_merge_tuples(btree_config *cfg,
                   ...
                   char *new_data)
{
   return cfg->data_cfg->merge_tuples(key, old_data, new_data);
}
                ...
                uint32        k)
{
   ...
   uint8 *table;
   if (hdr->is_packed) {
      return ((char *)hdr) + sizeof(*hdr) + k * (btree_key_size(cfg) + btree_data_size(cfg));
   }
   table = btree_get_table(cfg, node);
   idx = table[k];
   return (char *)table + cfg->tuples_per_leaf * sizeof(uint8)
         + idx * (btree_key_size(cfg) + btree_data_size(cfg));
}
                ...
                uint64        *generation)
{
   ...
   *generation = node->hdr->generation++;
   ...
   {
      const uint64 data_size = btree_data_size(cfg);
      ...
      memmove(old_data, btree_data, data_size);
      memmove(btree_data, data, data_size);
   ...
   memmove(btree_get_data(cfg, node, idx), data, btree_data_size(cfg));
   node->hdr->num_entries++;
                       ...
                       uint16 pos)
{
   ...
   memmove(btree_get_data(cfg, node, pos), data, btree_data_size(cfg));
   node->hdr->num_entries++;
}
           ...
           bool            is_packed)
{
   ...
   if (!is_packed) {
      ...
      for (uint8 i = 0; i < cfg->tuples_per_leaf; i++) {
         table[i] = i;
      }
   }
                         ...
                         page_type     type)
{
   ...
   // We don't hold the root lock, but we only dealloc from here
             ...
             bool         *found)     // OUT
{
   ...
   btree_lookup_with_ref(cc, cfg, root_addr, &node, PAGE_TYPE_BRANCH, key,
                         &data, found);
   if (*found) {
      memmove(data_out, data, btree_data_size(cfg));
      btree_node_unget(cc, cfg, &node);
   }
}
                        ...
                        platform_stream_handle stream)
{
   ...
   if (btree_height(cfg, node) > 0) {
      ...
      for (uint64 i = 0; i < btree_num_entries(cfg, node); i++) {
         ...
         btree_data_to_string(cfg, btree_get_data(cfg, node, i), data_string);
                  ...
                  uint64        extent_size)
{
   ...
   packed_tuple_size = data_cfg->key_size + data_cfg->data_size;
