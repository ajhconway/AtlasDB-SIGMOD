...
typedef bool (*mini_allocator_for_each_fn)(cache *cc, page_type type, uint64
      base_addr, uint64 *pages_outstanding);
                     ...
                     uint64                      meta_head)
{
   ...
   do {
      ...
      for (i = 0; i < hdr->pos; i++) {
         ...
         data_key_to_string(entry->start_key, start_key_str, 24);
         char end_key_str[256];
         data_key_to_string(entry->end_key, end_key_str, 24);
