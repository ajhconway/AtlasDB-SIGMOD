                      ...
                      test_async_lookup *async_lookup)
{
   ...
   uint64 data_size = spl->cfg.data_cfg->data_size;
                            ...
                            uint64                      end_index)
{
   ...
   while (SUCCESS(iterator_at_end((iterator *)range_itor, &at_end)) &&
      ...
      iterator_get_curr((iterator *)range_itor,
                        (char **)&splinter_key, (char **)&splinter_data,
                        &type);
      splinter_key = be64toh(*splinter_keybuf);
           ...
           void *keybuf)        // OUT
{
   ...
   switch (type) {
      ...
      case VERIFY_RANGE_ENDPOINT_RAND: {
         ...
         uint64 pos;
         pos = num_keys / 5 + (random_next_uint64(prg) %
                       ...
                       int64            maxdelta)
{
   ...
   for (i = 0; i < num_messages; i++) {
      ...
      key = minkey + (((key - minkey) + mindelta +
               (random_next_uint64(prg) % (maxdelta - mindelta + 1))) % (maxkey - minkey + 1));
                   ...
                   uint32                max_async_inflight)
{
   ...
   splinter_id *splinters =
      platform_aligned_malloc(hid, PLATFORM_CACHELINE_SIZE,
                              sizeof(splinter_id) * num_tables);
   platform_assert(splinters != NULL);
   test_async_lookup *async_lookup;
   if (max_async_inflight > 0) {
      async_ctxt_init(hid, max_async_inflight, cfg->data_cfg->data_size,
                      &async_lookup);
   } else {
      async_lookup = NULL;
   }
   ...
   for (uint8 idx = 0; idx < num_tables; idx++) {
      ...
      splinters[idx] = test_generate_splinter_id();
   ...
   while (total_inserts < num_inserts) {
      ...
      platform_log("Round i=%8lu, op=%2d, nummsgs = %8d, minkey=%8lu, maxkey=%8lu, mindelta = %8ld, maxdelta=%8ld\n",
            i,
            op,
            num_messages,
            minkey, maxkey,
      ...
      for (uint8 idx = 0; idx < num_tables; idx++) {
         ...
         splinter_handle *spl = spl_tables[idx];
         test_splinter_shadow_tree *shadow = shadows[idx];
         splinter_id spl_id = splinters[idx];
