...
rc_allocator_init_meta_page(rc_allocator *al)
{
    ...
    * extent.
    */
   platform_assert((1 + MAX_SPLINTER_TABLES_PER_DISK) * al->cfg->page_size
                    <= al->cfg->extent_size);
   ...
   memset(al->meta_page->splinters, SPLINTER_INVALID_ID,
                            ...
                            uint64       *addr)
{
   ...
   for (uint8 idx = 0; idx < MAX_SPLINTER_TABLES_PER_DISK; idx++) {
      if (al->meta_page->splinters[idx] == splinter_id) {
                              ...
                              uint64 *addr)
{
   ...
   platform_mutex_lock(&al->lock);
   for (uint8 idx = 0; idx < MAX_SPLINTER_TABLES_PER_DISK; idx++) {
      if (al->meta_page->splinters[idx] == SPLINTER_INVALID_ID) {
         // assign the first available slot and update the on disk metadata.
         al->meta_page->splinters[idx] = splinter_id;
         *addr = (1 + idx) * al->cfg->page_size;
                               ...
                               splinter_id splinter_id)
{
   ...
   for (uint8 idx = 0; idx < MAX_SPLINTER_TABLES_PER_DISK; idx++) {
       ...
       */
      if (al->meta_page->splinters[idx] == splinter_id) {
         al->meta_page->splinters[idx] = SPLINTER_INVALID_ID;
         al->meta_page->checksum =
            platform_checksum128(al->meta_page,
                                 sizeof(al->meta_page->splinters),
                                 SPLINTER_COMMON_CSUM_SEED);
