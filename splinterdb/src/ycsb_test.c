...
void record_latency(latency_table table, uint64_t latency)
{
...
void add_latency_table(latency_table dest, latency_table table)
{
   uint64_t exponent;
   uint64_t mantissa;
...
uint64_t num_latencies(latency_table table)
{
   uint64_t count = 0;
   uint64_t exponent;
   uint64_t mantissa;
...
uint64_t compute_latency(uint64_t exponent, uint64_t mantissa)
{
   if (exponent == 0)
      return mantissa;
   else if (exponent == 1)
...
uint64_t total_latency(latency_table table)
{
   uint64_t total = 0;
   uint64_t exponent;
   uint64_t mantissa;
   ...
   for (exponent = 0; exponent < LATENCY_EXPONENT_LIMIT; exponent++)
      for (mantissa = 0; mantissa < LATENCY_MANTISSA_LIMIT; mantissa++)
         total += table[exponent][mantissa]
            * compute_latency(exponent, mantissa);
   return total;
}

double mean_latency(latency_table table)
{
...
uint64_t min_latency(latency_table table)
{
   uint64_t exponent;
   uint64_t mantissa;
...
uint64_t max_latency(latency_table table)
{
   int64_t exponent;
   int64_t mantissa;
...
uint64_t ith_latency(latency_table table, uint64_t rank)
{
   uint64_t exponent;
   uint64_t mantissa;
...
uint64_t latency_percentile(latency_table table, float percent)
{
   return ith_latency(table, (percent / 100) * num_latencies(table) );
}

void print_latency_table(platform_stream_handle stream,
                         latency_table          table)
{
   uint64_t exponent;
   uint64_t mantissa;
      ...
      for (mantissa = 0; mantissa < LATENCY_MANTISSA_LIMIT; mantissa++)
         if (started || table[exponent][mantissa]) {
            platform_log_stream("%20lu %20lu\n",
                compute_latency(exponent, mantissa), table[exponent][mantissa]);
            started = 1;
...
void write_latency_table(char *filename, latency_table table)
{
...
void print_latency_cdf(platform_stream_handle stream, latency_table table)
{
...
void write_latency_cdf(char *filename, latency_table table)
{
   FILE *stream = fopen(filename, "w");
   platform_assert(stream != NULL);
         ...
         switch(ops->cmd) {
            ...
            case 's': {
                         ...
                         const size_t size_needed =
                            ops->range_len * (MAX_KEY_SIZE + MAX_DATA_SIZE);
                         if (range_buffer_size < size_needed) {
                            if (range_buffer) {
                               platform_free(hid, range_buffer);
                            }
                            range_buffer_size =
                               range_buffer_size * 2 < size_needed
                               ? size_needed
                               : range_buffer_size * 2;
                            range_buffer =
                               TYPED_ARRAY_MALLOC(hid,
                                                  range_buffer,
                                                  range_buffer_size);
...
ycsb_thread(void *arg)
{
   ...
   __sync_val_compare_and_swap(&params->times.earliest_thread_start_time,
         0, start_time);
   ...
   uint64_t thread_cputime = SEC_TO_NSEC(end_thread_cputime.tv_sec)
...
void unload_ycsb_log(ycsb_op *log, uint64 num_ops)
{
               ...
               uint64      *memory_bytes_out)
{
   ...
   *memory_bytes_out = MiB_TO_B(strtoull(argv[5], NULL, 0));
   ...
   for (lognum = 0; lognum < num_threads; lognum++) {
      ...
      ret = platform_thread_create(&params[lognum].thread,
            FALSE, parse_ycsb_log_file, req, hid);
   ...
   log_size_bytes += num_threads * max_range_len * (YCSB_KEY_SIZE + YCSB_DATA_SIZE);
...
void unload_ycsb_logs(ycsb_phase *phases, uint64 nphases)
{
   int i, j;
...
void compute_log_latency_tables(ycsb_log_params *params)
{
   ...
   sum_latency_tables(params->tables.all_queries,
         params->tables.pos_queries, params->tables.neg_queries);
}

void compute_phase_latency_tables(ycsb_phase *phase)
{
   uint64_t i;
   for (i = 0; i < phase->nlogs; i++)
...
void compute_all_phase_latency_tables(ycsb_phase *phases, int num_phases)
{
   uint64_t i;
...
void compute_phase_statistics(ycsb_phase *phase)
{
   uint64_t i;

   for (i = 0 ; i < phase->nlogs; i++) {
      if (i == 0 || phase->params[i].times.earliest_thread_start_time
            < phase->times.earliest_thread_start_time) {
         phase->times.earliest_thread_start_time
            = phase->params[i].times.earliest_thread_start_time;
      }
      if (i == 0 || phase->params[i].times.last_thread_finish_time
...
void compute_all_phase_statistics(ycsb_phase *phases, int num_phases)
{
   uint64_t i;
   for (i = 0; i < num_phases; i++)
...
void compute_all_report_data(ycsb_phase *phases, int num_phases)
{
...
{
   char filename[1024];
   snprintf(filename, sizeof(filename), "%s.%02lu.%s.latency.df",
         phase_name, lognum, operation_name);
...
{
   char filename[1024];
   snprintf(filename, sizeof(filename), "%s.%02lu.%s.latency.cdf",
         phase_name, lognum, operation_name);
...
{
   char filename[1024];
   snprintf(filename, sizeof(filename), "%s.%s.latency.df",
...
{
   char filename[1024];
   snprintf(filename, sizeof(filename), "%s.%s.latency.cdf",
         phase_name, operation_name);
   write_latency_cdf(filename, table);
}

void write_phase_latency_tables(ycsb_phase *phase)
{
   uint64_t i;
   for (i = 0; i < phase->nlogs; i++) {
      write_log_latency_table(phase->name, i, "pos_query",
            phase->params[i].tables.pos_queries);
      write_log_latency_table(phase->name, i, "neg_query",
            phase->params[i].tables.neg_queries);
      write_log_latency_table(phase->name, i, "all_query",
            phase->params[i].tables.all_queries);
      write_log_latency_table(phase->name, i, "delete",
            phase->params[i].tables.deletes);
      write_log_latency_table(phase->name, i, "insert",
            phase->params[i].tables.inserts);
      write_log_latency_table(phase->name, i, "update",
            phase->params[i].tables.updates);
      write_log_latency_table(phase->name, i, "scan",
            phase->params[i].tables.scans);
   }

   for (i = 0; i < phase->nlogs; i++) {
      write_log_latency_cdf(phase->name, i, "pos_query",
            phase->params[i].tables.pos_queries);
      write_log_latency_cdf(phase->name, i, "neg_query",
            phase->params[i].tables.neg_queries);
      write_log_latency_cdf(phase->name, i, "all_query",
            phase->params[i].tables.all_queries);
      write_log_latency_cdf(phase->name, i, "delete",
            phase->params[i].tables.deletes);
      write_log_latency_cdf(phase->name, i, "insert",
            phase->params[i].tables.inserts);
      write_log_latency_cdf(phase->name, i, "update",
            phase->params[i].tables.updates);
      write_log_latency_cdf(phase->name, i, "scan",
            phase->params[i].tables.scans);
   }

   write_phase_latency_table(phase->name, "pos_query",
         phase->tables.pos_queries);
   write_phase_latency_table(phase->name, "neg_query",
         phase->tables.neg_queries);
   write_phase_latency_table(phase->name, "all_query",
         phase->tables.all_queries);
...
print_operation_statistics(platform_log_handle  output,
                           ...
                           latency_table        table)
{
   platform_handle_log(output, "%s_count: %lu\n",
         operation_name, num_latencies(table));
   platform_handle_log(output, "%s_total_latency: %lu\n",
         operation_name, total_latency(table));
   platform_handle_log(output, "%s_min_latency: %lu\n",
         operation_name, min_latency(table));
   platform_handle_log(output, "%s_mean_latency: %lf\n",
         operation_name, mean_latency(table));
   platform_handle_log(output, "%s_median_latency: %lu\n",
         operation_name, latency_percentile(table, 50.0));
   platform_handle_log(output, "%s_99.0_latency: %lu\n",
         operation_name, latency_percentile(table, 99));
   platform_handle_log(output, "%s_99.5_latency: %lu\n",
         operation_name, latency_percentile(table, 99.5));
   platform_handle_log(output, "%s_99.9_latency: %lu\n",
         operation_name, latency_percentile(table, 99.9));
   platform_handle_log(output, "%s_max_latency: %lu\n",
         operation_name, max_latency(table));

   platform_handle_log(output, "%s_000_latency: %lu\n",
         operation_name, min_latency(table));
   int i;
   for (i = 5; i < 100; i += 5) {
      platform_handle_log(output, "%s_%03d_latency: %lu\n",
            operation_name, i, latency_percentile(table, i));
   }
   platform_handle_log(output, "%s_100_latency: %lu\n",
         operation_name, max_latency(table));
}
                      ...
                      latency_tables      *tables)
{
   ...
   platform_handle_log(output, "sum_of_wall_clock_times: %lu\n",
         times->sum_of_wall_clock_times);
   platform_handle_log(output, "sum_of_cpu_times: %ld\n",
         times->sum_of_cpu_times);
   platform_handle_log(output, "mean_overall_latency: %f\n",
         total_ops ? 1.0 * times->sum_of_wall_clock_times / total_ops : 0);
   platform_handle_log(output, "mean_overall_throughput: %f\n",
...
{
   char filename[1024];
   snprintf(filename, sizeof(filename), "%s.%02lu.statistics",
         phase_name, lognum);
   FILE *output = fopen(filename, "w");
   platform_assert(output != NULL);

   print_statistics_file(output, params->total_ops,
         &params->times, &params->tables);

   fclose(output);
}

void write_phase_statistics_files(ycsb_phase *phase)
{
   uint64_t i;
   for (i = 0; i < phase->nlogs; i++)
   ...
   print_statistics_file(output, phase->total_ops,
         &phase->times, &phase->tables);

   fclose(output);
}

void write_all_reports(ycsb_phase *phases, int num_phases)
{
   uint64_t i;
   for (i = 0; i < num_phases; i++) {
...
ycsb_test(int argc, char *argv[])
{
   ...
   if (data_cfg->data_size != YCSB_DATA_SIZE) {
      rc = STATUS_BAD_PARAM;
   ...
   uint64 buffer_bytes = MiB_TO_B(128);
   ...
   if (use_existing) {
      ...
      spl = splinter_mount(splinter_cfg, (allocator *)&al, (cache *)cc,
                           ts, test_generate_splinter_id(), hid);
      platform_assert(spl);
   } else {
