...
typedef struct splinter_config {
   ...
   quick_filter_config  filter_cfg;
...
typedef struct splinter_stats {
   ...
   uint64 lookup_time_ns;
   uint64 memtable_lookup_time_ns;
   uint64 filter_lookup_time_ns[SPLINTER_MAX_HEIGHT];
   uint64 branch_lookup_time_ns[SPLINTER_MAX_HEIGHT];
} PLATFORM_CACHELINE_ALIGNED splinter_stats;

#if 0
// uint32 aligned to conserve space on disk
typedef struct splinter_tree {
   uint64       root_addr;
   quick_filter filter;
} PACKED UINT32_ALIGNED splinter_tree;
_Static_assert(sizeof(splinter_tree) == sizeof(uint64) + sizeof(quick_filter),
      "splinter_tree has padding");

typedef struct splinter_branch {
   splinter_tree trees[NUM_DATA_TYPES];
} splinter_branch;
_Static_assert(
      sizeof(splinter_branch) == NUM_DATA_TYPES * sizeof(splinter_tree),
      "splinter_branch has padding");
#endif
...
struct splinter_handle {
   ...
   // task system
   task_system       *ts;  // currently not durable
...
typedef enum {
   async_state_start,
   async_state_get_root,                 // re-entrant state
   async_state_lookup_memtable,
   async_state_find_pivot,
   async_state_branch_lookup,
   async_state_filter_lookup_start,
   async_state_filter_lookup,            // re-entrant state
   async_state_btree_lookup_start,
   async_state_btree_lookup,             // re-entrant state
   async_state_next_branch,
   async_state_get_next_trunk,           // re-entrant state
   async_state_unget_curr_trunk,
   async_state_look_in_leaf,
   async_state_leaf_done,
...
typedef struct splinter_async_ctxt {
   ...
   union {
      quick_async_ctxt         quick_ctxt;    // Filter async context
...
static inline uint64
splinter_data_size(splinter_handle *spl)
{
   return spl->cfg.data_cfg->data_size;
}
...
static inline void
splinter_data_to_string(splinter_handle *spl,
                        const char      *data,
                        char            *str)
{
   btree_data_to_string(&spl->cfg.btree_cfg, data, str);
}
