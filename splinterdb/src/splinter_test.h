                  ...
                  char        **argv_p[])
{
   ...
   temp_config *temp_cfg = TYPED_ARRAY_MALLOC(platform_get_heap_id(), temp_cfg,
                                              num_config);
   for (i = 0; i < argc; i++) {
      if (0) {
      config_set_mib("tree-size", cfg, tree_size) {
         ...
         *argv_p += 2;
         ...
         for (uint8 idx = 0; idx < num_config; idx++) {
            ...
            else if (STRING_EQUALS_LITERAL(temp_cfg[idx].key_type, "semiseq"))
